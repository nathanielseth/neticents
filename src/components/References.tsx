...
const References = () => {
  ...
  const getHoverClasses = (category: ReferenceLink['category']): string => {
    const hoverStyles: Record<ReferenceLink['category'], string> = {
      ...
      philhealth: 'hover:bg-purple-50 dark:hover:bg-purple-400/15 hover:text-purple-700 dark:hover:text-purple-400',
      pagibig: 'hover:bg-yellow-50 dark:hover:bg-yellow-400/15 hover:text-yellow-700 dark:hover:text-yellow-400',
      tax: 'hover:bg-red-50 dark:hover:bg-red-400/15 hover:text-red-700 dark:hover:text-red-400',
      other: 'hover:bg-blue-50 dark:hover:bg-blue-400/15 hover:text-blue-700 dark:hover:text-blue-400',
    }
    return hoverStyles[category]
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {referenceLinks.map((link, index) => (
            <a
              ...
              className={`block p-3 rounded-lg border bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-600 ${getHoverClasses(link.category)}`}
            >
