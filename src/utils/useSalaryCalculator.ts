...
import type { Sector, WorkSchedule } from '../types'
import {
...
import { handleInput, parseFormattedNumber, numberFormat } from './format'
...
interface UseSalaryCalculatorReturn {
  ...
  handleAllowanceChange: (e: React.ChangeEvent<HTMLInputElement>) => void
  handleSectorChange: (sector: Sector) => void
  handleWorkScheduleChange: (schedule: WorkSchedule) => void
  handleOvertimeHoursChange: (e: React.ChangeEvent<HTMLInputElement>) => void
  handleNightDifferentialHoursChange: (e: React.ChangeEvent<HTMLInputElement>) => void
  getDeMinimisHelperText: () => string
}
...
export const useSalaryCalculator = (
  ...
  const [activeSector, setActiveSector] = useState<Sector>('private')
  const [workSchedule, setWorkSchedule] = useState<WorkSchedule>('mon-fri')
  ...
  const getDeMinimisHelperText = useCallback((): string => {
  ...
  const calculatePremiumPay = useCallback(
    (
      ...
      const nightOvertimeRate = 1.375
       ...
       // hours that qualify for both OT and night diff
      ...
      const regularOvertimePay = regularOvertimeHours * hourlyRate * overtimeRate
  ...
  const calculate = useCallback(
    (
      ...
      const taxableAllowance = Math.max(0, allowanceAmount - monthlyDeMinimisLimit)
      ...
      if (sector === 'private' || sector === 'selfemployed') {
        ...
        contributions.philHealth = computePhilHealth(taxableSalary, isSelfEmployed)
      ...
      const totalContributions =
        sector === 'public'
          ? contributions.gsis + contributions.philHealth + contributions.pagIbig
          : contributions.sss +
            contributions.mpf +
  ...
  const parseValue = useCallback((formattedValue: string): number => {
  ...
  const handleSalaryChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      calculate(salary, allowanceAmount, activeSector, otHours, ndHours, workSchedule)
    },
    [
      allowance,
      overtimeHours,
  ...
  const handleSectorChange = useCallback(
    (sector: Sector) => {
      ...
      const otHours = parseValue(overtimeHours)
      const ndHours = parseValue(nightDifferentialHours)
      calculate(salary, allowanceAmount, sector, otHours, ndHours, workSchedule)
    },
    [
      monthlySalary,
      allowance,
  ...
  const handleWorkScheduleChange = useCallback(
    (schedule: WorkSchedule) => {
      ...
      const otHours = parseValue(overtimeHours)
      const ndHours = parseValue(nightDifferentialHours)
      calculate(salary, allowanceAmount, activeSector, otHours, ndHours, schedule)
    },
    [
      monthlySalary,
      allowance,
