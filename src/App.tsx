...
const App = () => {
  ...
  const [nightDifferentialHours, setNightDifferentialHours] = useState<string | number>('')
  ...
  const handleDownload = async () => {
    ...
    try {
      const taxData = {
        ...
        allowanceTaxable: Number(allowance) > 7500,
        sector,
      ...
      const url = await generateTaxSummaryPDF(taxData)
      setPdfUrl(url)
    ...
    <div className="min-h-screen bg-[#fbfcfd] dark:bg-gray-900 text-gray-900 dark:text-white flex flex-col relative">
      ...
      <footer className="w-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm p-4 text-center text-sm">
        <div className="flex items-center justify-center gap-1">
          <span className="text-gray-400 dark:text-gray-500">Developed by </span>
          <a
